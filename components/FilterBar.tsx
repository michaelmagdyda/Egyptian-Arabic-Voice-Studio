...
import { Search, Sparkles, ChevronDown, Mic, LayoutGrid, GalleryHorizontalEnd, Sun, Moon, Radio } from 'lucide-react';
...
const FilterBar: React.FC<FilterBarProps> = ({ 
  ...
  return (
    <div className="relative z-50 w-full bg-white/85 dark:bg-zinc-900/85 backdrop-blur-xl border-b border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 h-16">
        <div className="flex items-center justify-between h-full gap-2 sm:gap-4">
            
          {/* Left: Brand + Active Voice-Over Pill + AI Casting */}
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <div className="flex items-center gap-2 group select-none">
              ...
              <h1 className="hidden lg:block text-base sm:text-lg font-bold tracking-tight text-zinc-900 dark:text-white font-display whitespace-nowrap">
                Gemini Voice Library
              </h1>
            </div>

            {/* Active Voiceover Studio Quick Button */}
            {hasActiveVoiceover && onOpenVoiceoverStudio && (
              <button 
                onClick={onOpenVoiceoverStudio}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/50 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200/80 dark:border-indigo-800/60 rounded-full text-xs font-semibold shadow-2xs transition-all hover:scale-105 active:scale-95 shrink-0"
                title="Open Egyptian Arabic Voice-Over Studio"
              >
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                <span className="font-arabic font-bold text-xs">إعداد خدام</span>
                <span className="hidden md:inline font-sans text-[11px] opacity-80">(Egyptian Voice-Over)</span>
              </button>
            )}

            <button 
              onClick={onOpenAiCasting}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-zinc-900 dark:bg-indigo-600 hover:bg-zinc-800 dark:hover:bg-indigo-500 text-white rounded-full text-xs sm:text-sm font-medium shadow-md transition-all hover:scale-105 active:scale-95 group shrink-0"
            >
